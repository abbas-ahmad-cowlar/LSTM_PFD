...
| §8.6 | XAI alignment (SHAP/IG energy in PHYSICS.md frequency bands) + MC-dropout calibration | laptop | **BUILDING NOW** (`scripts/run_xai_calibration.py`). Infra confirmed usable: `packages/core/explainability/{integrated_gradients,uncertainty_quantification}.py` are pure-torch (no captum/shap needed). best-vanilla=resnet18 ckpt (`results/benchmark/deep/`), best-physics=fixed pc_cnn w0.3 ckpt (`D:\Libraries\...fixed_full...`). Runs on CPU. C4 contribution — now the main physics-knowledge angle |
