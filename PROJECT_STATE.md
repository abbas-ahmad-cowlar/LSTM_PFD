...
| §8.6 | XAI alignment (IG energy in PHYSICS.md frequency bands) + MC-dropout calibration | laptop | **DONE** (`scripts/run_xai_calibration.py`; `results/xai_alignment/`, `results/uncertainty/`, note `findings_8_6.md`). resnet18 vs pc_cnn share IDENTICAL backbone (3.85M params) → isolates physics-loss effect. **8.6a**: physics model's attributions more physics-band-aligned (specificity ratio 0.849 vs 0.716) — modest C4 positive (caveat: both <1, control-band-sensitive). **8.6b**: pc_cnn better calibrated (ECE clean 0.022 vs 0.028, 5dB 0.018 vs 0.027); both hit 100% acc @50% coverage. Caveat: single checkpoint (seed-average for paper). Fixed a real MC-dropout BN bug en route |
