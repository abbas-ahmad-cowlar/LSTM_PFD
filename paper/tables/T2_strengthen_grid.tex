...
% results/p7_strengthen/p7_strengthen_record_level.json  (every number traced).
\begin{table}[t]
  \centering
  \caption{Pre-registered \S8.8 $n{=}12$ grid: per-arm clean$\rightarrow$5\,dB
    ...
    replicate. The within-seed McNemar is deliberately \emph{not} reported here
    as evidence (see \S\ref{sec:caution}).}
  \label{tab:grid}
