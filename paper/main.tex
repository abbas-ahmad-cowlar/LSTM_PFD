...
(\texttt{physics\_constrained\_cnn}, PC-CNN): a ResNet1D backbone whose physics
...
shaft speed plus absolute broadband bands (\texttt{docs/PHYSICS.md}\,\S4) --- the
band penalty is $\mathrm{pen}_b = \mathrm{relu}\!\left(1 - \mathrm{frac}_b /
H_{\mathrm{ref}}[c][b]\right)$, where $\mathrm{frac}_b$ is the signal's energy
fraction in band $b$ and $H_{\mathrm{ref}}$ is the measured healthy-class
per-band energy fraction (\texttt{healthy\_reference.json}, computed from
1{,}120 training healthy windows). Per-class penalties are averaged over the
class's bands and weighted by the softmax probability of $c$. The loss is
differentiable and unit-tested, and the per-sample operating conditions are
wired into training. We stress-test this one method across the five regimes
where physics priors are theorized to help: noise robustness, data-efficiency,
severity out-of-distribution, interpretability, and calibration.
...
mean degradation and the most robust seeds, while \emph{correct} physics is the
weakest of the three $w{=}1.0$ arms. The data therefore support neither a
...
\begin{figure}[t]
  ...
  \caption{Per-seed clean$\rightarrow$5\,dB record-level degradation (528
    ...
    $\ge$10/12 seeds, and the non-physical random-band control is the most
    robust; the seed variance that produced an apparent $n{=}3$ benefit is
    visible. Source:
...
\label{sec:caution}
\paragraph{An effect significant at $n{=}3$ dissolves at $n{=}12$.}
...
null ($3.54$ vs $3.47$\,pt, Wilcoxon $p=0.79$). The within-seed
...
conditions on a single seed --- the one where every arm happens to be robust ---
and treats the five windows per record as independent. We report it only as the
statistic that fooled the $n{=}3$ analysis, never as evidence of an effect; the
pre-registered seed-level test governs.
...
\begin{figure}[t]
  ...
  \caption{\textbf{Headline.} The apparent noise-robustness benefit is a seed
    ...
    seeds grow. The within-seed McNemar is shown nowhere as evidence. Source:
    \texttt{results/p7\_strengthen/p7\_strengthen\_record\_level.json}.}
...
regularizer of equal strength. In this study the matched-strength non-physics
control was decisive: it was the most robust arm, ruling out even a generic
spectral-regularizer reading.
